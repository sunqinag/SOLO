...
from ..file_operator.bz_path import get_file_path
from ..image_processing_operator import tensorflow_image_processing_map, \
    tensorflow_image_processing


def get_img_label_list(img_dir, label_dir, ret_full_path=True):
    img_file_path_list = np.sort(
        get_file_path(img_dir, ret_full_path=ret_full_path))

    if label_dir is not None:
        label_file_path_list = np.sort(
            get_file_path(label_dir, ret_full_path=ret_full_path))
        if len(img_file_path_list) != len(label_file_path_list):
            raise ValueError('图像数目与标签数目不一致!')
...
class ImageLabelDataSetBaseClass:
    def __init__(self, img_list, label_list, channels_list, img_format_list,
                 height_width, crop_height_width=(400, 400),
                 batch_size=1, num_epochs=1, shuffle=False, mode='train',
                 task='classification'):
        """
        dataset基类
        ...
        :param img_format_list: 解码图像的格式,目前支持bmp,jpg,jpeg,png,gif
    ...
    def create_dataset(self):
        ...
        dataset_src = (
        {'img_list': self.img_path_list}, {'label_list': self.label_list})
        dataset = tf.data.Dataset.from_tensor_slices(dataset_src)
        dataset_map = dataset.map(self._read_img_label)
        dataset_map = dataset_map.map(self._preprocess_image_map_outer)
        if self.shuffle:
        ...
        dataset = dataset.prefetch(buffer_size=100 * self.batch_size)
    ...
    def _preprocess_image_map_outer(self, data_dict):
        ...
        image_map = tf.reshape(image, [-1, image.shape[0], image.shape[1],
                                       image.shape[2]])
        label_map = None

        if self.task == 'segmentation' and (
                self.mode == 'train' or self.mode == 'evaluate'):
            label_map = tf.reshape(label, [-1, label.shape[0], label.shape[1],
                                           label.shape[2]])
        if self.mode == 'train':
            image_map, label_map = self.preprocess_image_map(
                {'img': image_map, 'label': label_map})

        image_map = tf.image.resize_bilinear(image_map,
                                             size=[self.crop_height_width[0],
                                                   self.crop_height_width[1]])
        image_map = tf.reshape(image_map,
                               shape=[self.crop_height_width[0],
                                      self.crop_height_width[1],
    ...
    def _read_img_label(self, img_path_list, labels):
        ...
        img = tensorflow_image_processing.read_image_and_resize_tf(
            img_list, self.channels_list[0], self.img_format_list[0],
        ...
        if self.task == 'segmentation' and (
            ...
            label = tensorflow_image_processing.read_image_and_resize_tf(
                label_list, self.channels_list[1], self.img_format_list[1],
                height=self.height_width[0], width=self.height_width[1],
    ...
    def _check_params(self):
        ...
        if isinstance(self.mode, str):
            if self.mode.lower() not in mode_tuple:
                raise ValueError(
                    "mode　必须为'train','evaluate','predict','TRAIN','EVALUATE','PREDICT',或者1,2,3！")
        ...
        if isinstance(self.task, str):
            if self.task.lower() not in task_tuple:
                raise ValueError(
                    "task　必须为'classification','segmentation','CLASSIFICATION','SEGMENTATION',或者1,2！")
        ...
        if (self.mode == 'train' or self.mode == 'evaluate'):
            if (not isinstance(self.img_path_list,
                               np.ndarray) and self.img_path_list is None) or \
                    (not isinstance(self.label_list,
                                    np.ndarray) and self.label_list is None):
        ...
        if self.task == 'segmentation' and (
                len(self.channels_list) != 2 or len(self.img_format_list) != 2):
...
class ImageLabelDataSet(ImageLabelDataSetBaseClass):
    ...
    def preprocess_image_map(self, data_dict):
        ...
        image, label = tensorflow_image_processing_map.random_crop_or_pad_image_and_label_tf_map(
            image, label=label, crop_height=self.crop_height_width[0],
            crop_width=self.crop_height_width[1])
        # image,label=tensorflow_image_processing_map.flip_image_and_label_tf_map(image,label)
        image, label = tensorflow_image_processing_map.random_rotate_image_and_label_tf_map(
            image, label, max_angle=15)
        image, label = tensorflow_image_processing_map.flip_image_and_label_tf_map(
            image, label)
        ...
        image, label = tensorflow_image_processing_map.random_translate_image_and_label_tf_map(
            image, label, max_dx=15,
            max_dy=15)
        #
        ...
        image = tensorflow_image_processing_map.add_random_noise_tf_map(image,
                                                                        0.5, 50)
