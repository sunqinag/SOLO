...
import base_classify_estimator
...
class AlexnetEstimator(base_classify_estimator.BaseClassifyEstimator):
    ...
    def network(self, img_batch, is_training):
        ...
        :return:
        """
        if img_batch.shape[-3:] != self.img_shape:
    ...
    def __check_param(self):
        ...
        if len(self.regularizer_scale) != 2:
            raise ValueError('输入的regularizer_scale长度应为2！')
        if self.regularizer_scale[0] < 0 or self.regularizer_scale[1] < 0:
            raise ValueError('输入的正则项系数必须大于等于0')
        if isinstance(self.label_weight, list) and len(
                self.label_weight) != self.class_num:
        ...
        if len(
                self.custom_conv_filters) != 0 and self.alexnet_layers_used_num > 5:
            raise ValueError(
                'costum网络中有卷积层时alexnet层只能取卷积层，即alexnet_layers_used_num<=5')
        if self.alexnet_layers_used_num > 7 or self.alexnet_layers_used_num < 1:
            raise ValueError('alexnet_layers_used_num得取值范围为1~7')

    def __conv_layers(self, scope_name, input_layer, conv_kernel_sizes,
                      conv_strides, conv_filters, activation_fn,
                      is_training, split_layers_indices=(), pool_indices=(),
        ...
        for conv_layer_index in range(1, conv_layers_num + 1):
            with tf.variable_scope(
                    scope_name + '/conv' + str(conv_layer_index)):
                if conv_layer_index in split_layers_indices:
                    conv_groups = tf.split(axis=3, value=last_layer,
                                           num_or_size_splits=2)
                    with tf.variable_scope('up'):
                        conv_up = tf.contrib.layers.conv2d(
                            inputs=conv_groups[0],
                            num_outputs=int(
                                conv_filters[conv_layer_index - 1] / 2),
                    ...
                    last_layer = tf.contrib.layers.conv2d(
                        ...
                        normalizer_fn=tf.contrib.layers.batch_norm,
                        normalizer_params={'is_training': is_training})
                self.layer_info_dict[scope_name + '/conv' + str(
                    conv_layer_index)] = last_layer.get_shape().as_list()
            if conv_layer_index in pool_indices:
                with tf.variable_scope(
                        scope_name + '/pool' + str(conv_layer_index)):
    ...
    def __dense_layers(self, scope_name, input_layer, dense_filters,
                       dropout_indices, activation_fn,
                       is_training):
        """
        ...
        :param dropout_rate:  dropout层索引对应层dropout rate大小
        :param is_training:  是否在训练模式下调用
        :return:
        """
        ...
        for dense_layer_index in range(1, dense_layers_num + 1):
            with tf.variable_scope(
                    scope_name + '/dense' + str(dense_layer_index)):
                if dense_layer_index in dropout_indices:
                    last_layer = tf.contrib.layers.dropout(
                        inputs=last_layer,
                        keep_prob=0.5,
                        is_training=is_training)
                if dense_layer_index == dense_layers_num and dense_filters[
                    dense_layer_index - 1] == self.class_num:
                    activation_fn = None
                ...
                self.layer_info_dict[scope_name + '/conv' + str(
                    dense_layer_index)] = last_layer.get_shape().as_list()
    ...
    def __alexnet(self, img_batch, is_training):
        ...
        if self.alexnet_layers_used_num > alexnet_conv_layers_num:
            ...
            conv_output = self.__conv_layers('alexnet', img_batch,
                                             conv_kernel_sizes, conv_strides,
                                             conv_filters,
                                             alexnet_activation_fn, is_training,
                                             split_layers_indices, pool_indices,
                                             pool_size, pool_strides)
            alexnet_output = self.__dense_layers('alexnet', conv_output,
                                                 dense_filters, dropout_indices,
                                                 alexnet_activation_fn,
                                                 is_training)
        else:
            conv_filters = conv_filters[0: self.alexnet_layers_used_num]
            conv_strides = conv_strides[0: self.alexnet_layers_used_num]
            conv_kernel_sizes = conv_kernel_sizes[
                                0: self.alexnet_layers_used_num]
            alexnet_output = self.__conv_layers('alexnet', img_batch,
                                                conv_kernel_sizes, conv_strides,
                                                conv_filters,
                                                alexnet_activation_fn,
                                                is_training,
    ...
    def __custom_network(self, input_layer, is_training):
        ...
        """
        last_layer = input_layer
        if len(self.custom_conv_filters):
            last_layer = self.__conv_layers('custom_network', last_layer,
                                            self.custom_conv_kernel_sizes,
                                            self.custom_conv_kernel_sizes,
                                            self.custom_conv_filters,
                                            self.custom_activation_fns[0],
                                            is_training)
        if len(self.custom_dense_filters):
            last_layer = self.__dense_layers('custom_network', last_layer,
                                             self.custom_dense_filters,
                                             self.custom_dropout_indices,
                                             self.custom_activation_fns[1],
                                             is_training)
        return last_layer
    ...
    def __write_tensors(self):
        with open('tensor.json', 'w') as f:
            json.dump(self.layer_info_dict, f)
