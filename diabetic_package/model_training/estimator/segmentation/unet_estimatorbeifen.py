...
class UnetEstimator(base_segmentation_estimator.BaseSegmentationEstimator):
    def __init__(self,
                 ...
                 # 卷积特征提取层
                 feature_conv_filter_nums=(8, 16, 32, 64),
                 feature_conv_kernel_sizes=(3, 3, 3, 3),
                 feature_conv_strides=(1, 1, 1, 1),
                 # unet卷积层
                 ...
                 deconv_filter_nums=(64, 32, 21),
                 deconv_kernel_sizes=(3, 3, 3),
                 deconv_strides=(2, 2, 2),
                 deconv_padding='same',
                 deconv_activate_fn=tf.nn.relu,
                 model_dir='./model_dir',
                 transfer_checkpoint_path=''
                 ):
        """
        :param img_shape: 图像crop后shape
        :param class_num: 分类个数
        :param feature_conv_filter_nums: 特征提取层卷积核个数sigmoid
    ...
    def network(self, inputs, is_training):
        ...
        num = len(self.unet_conv_filter_nums) + 1
        # 开始建图
        heights = height * np.ones(num, dtype=int)
        widths = width * np.ones(num, dtype=int)
        ...
        for i in range(len(self.unet_conv_filter_nums)):
            ...
            heights[i + 1] = np.int32(result.get_shape().as_list()[1])
            widths[i + 1] = np.int32(result.get_shape().as_list()[2])
            result = tf.identity(result, name='pool_layer_' + str(i + 1))
            self.layer_info_dict[result.name] = result.get_shape().as_list()
        # # 反卷积层
        concate_unet_layer = None
            ...
            if i < (len(self.deconv_filter_nums) - 1):
                ...
                concate_layer = tf.concat(
                    [concate_unet_layer,
                     ...
                     result],
                    axis=3)
                result = tf.layers.conv2d(concate_layer,
    ...
    def get_loss(self, logits, labels):
        return self._get_background_and_foreground_weighted_loss(logits, labels)
