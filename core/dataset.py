...
class Dataset(object):
    ...
    def __next__(self):
        ...
        中间调用了 parse_annotation(annotation)函数，主要是 解析annotation，获取图像数据和标记框（做一些预处理）；
        以及调用了preprocess_true_boxes(bboxes)函数，主要是对标记框做一些预处理。'''
        with tf.device('/cpu:0'):
            ...
            batch_image = np.zeros(
                (self.batch_size, self.train_input_size, self.train_input_size, 3))  # 输入图像 1*448*448*3
